...
class Config:
    ...
    filter_size = 300
    num = 200
    mo_pic_path = './data/kobe.jpeg'
    sub_pic_path = './data/poster_downloads'
    save_path = './data/result.jpg'
...
def mapping_table(pic_folder):
    ...
    means, codes, pic_dic = {}, {}, {}
    for idx, pic in tqdm(enumerate(pic_list), desc='CODE'):  # 封装一个进度条
        if pic.split('.')[-1] in suffix:
            path = os.path.join(pic_folder, pic)  # 获得图片路径
            try:
                img = Image.open(path).convert('RGB').resize(Config.corp_size, Image.ANTIALIAS)  # 把每张子图都转为固定大小的图片
                codes[idx] = pic_code(np.array(img.convert('L').resize((8, 8), Image.ANTIALIAS)))  # 转为二值图像
                means[idx] = rgb_mean(np.array(img))  # RGB 三原色的均值
